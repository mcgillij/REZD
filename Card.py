...
class Card():
   ...
   # def containsHand(self, x, y):
    ...
    def contains2(self, x,y):  
        ...
        if x < ax or x > ax + self.sprite.width: return False
        if y < ay or y > ay + self.sprite.height: return False
        return True
        #p = Vec2(x,y)
    ...
    def contains(self, x, y):
        ...
        if self.isTapped:
            ...
            if x < ax or x > ax + self.sprite.height: return False
            if y < ay or y > ay + self.sprite.width: return False
        else:
            ax -= self.sprite.image.anchor_x
            ay -= self.sprite.image.anchor_y
    ...
    def getCardN(self):
        return CardN(self.cid, self.position, self.uuid, self.isTapped, self.faceUp, self.inHand, self.dirty)