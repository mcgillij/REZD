...
class TextureGraphicElementTemplate(UndefinedGraphicElementTemplate):
    ...
    def generate(self, color, batch, group):
        return TextureGraphicElement(self.theme, self.texture, color, batch, group)
    ...
    def write(self, f, indent=0):
        ...
        if hasattr(self.texture, "region"):
            f.write(
                ",\n"
                + " " * (indent + 2)
                + '"region": %s' % repr(list(self.texture.region))
            )
        f.write("\n" + " " * indent + "}")


class FrameTextureGraphicElementTemplate(TextureGraphicElementTemplate):
    def __init__(self, theme, texture, stretch, padding, width=None, height=None):
        TextureGraphicElementTemplate.__init__(
            self, theme, texture, width=width, height=height
        )
        self.stretch_texture = texture.get_region(*stretch).get_texture()
        x, y, width, height = stretch
        self.margins = (
            x,
            texture.width - width - x,  # left, right
            texture.height - height - y,
            y,
        )  # top, bottom
...
class FrameTextureGraphicElement:
    def __init__(
        self, theme, texture, inner_texture, margins, padding, color, batch, group
    ):
        self.x = self.y = 0
        self.width, self.height = texture.width, texture.height
        self.group = ThemeTextureGroup(texture, group)
    ...
    def get_needed_size(self, content_width, content_height):
        left, right, top, bottom = self.padding
        return (
            max(content_width + left + right, self.outer_texture.width),
