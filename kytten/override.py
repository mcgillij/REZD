...
def GetKyttenLayoutGroups(group):
    if group not in KYTTEN_LAYOUT_GROUPS:
        ...
        foreground_decoration_group = (
            pyglet.text.layout.TextLayoutForegroundDecorationGroup(2, top_group)
        )
        KYTTEN_LAYOUT_GROUPS[group] = (
            top_group,
            background_group,
            foreground_group,
            foreground_decoration_group,
...
class KyttenLabel(pyglet.text.Label):
    ...
    def teardown(self):
        ...
        if group is not None:
            ...
            self.top_group = (
                self.background_self
            ) = self.foreground_group = self.foreground_decoration_group = None
