...
class VerticalLayout(Widget):
    ...
    def layout(self, x, y):
        ...
        if self.align == HALIGN_RIGHT:
            ...
            for item in self.content:
                item.layout(x + self.width / 2 - item.width / 2, top - item.height)
                top -= item.height + self.padding
        else:  # HALIGN_LEFT
            for item in self.content:
...
class GridLayout(Widget):
    ...
    def __init__(self, content=[[]], anchor=ANCHOR_TOP_LEFT, padding=5, offset=(0, 0)):
        """
        ...
        assert (isinstance(content, list) or isinstance(content, tuple)) and (
            len(content) == 0
            or (isinstance(content[0], list) or isinstance(content[0], tuple))
        )
        Widget.__init__(self)
        self.content = content
        self.anchor = anchor
        ...
        for row in self.content:
            ...
            for cell in row:
                ...
                if cell is not None:
                    ...
                    cell.layout(
                        *GetRelativePoint(
                            placement, self.anchor, cell, self.anchor, self.offset
                        )
                    )
    ...
    def set(self, column, row, item):
        ...
        if len(self.content[row]) <= column:
            self.content[row] = list(self.content[row]) + [None] * (
                column - len(self.content[row]) + 1
            )
